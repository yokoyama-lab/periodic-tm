...
## この研究の見どころ — 3つの古典数学の「計算可能版」を測る

この研究は、よく知られた 3 つの古典数学が**計算可能性という制約の下で
どこまで生き残るかの精密測定**とみなせます。

1. **対合の積**（群論）— 対称群では任意の置換が 2 つの対合の積で書ける。
   では「計算可能」な世界でも書けるか？ → 有限位数なら書ける（本定理）。
2. **可逆力学系** — 「時間対称 ⟺ 2 つの鏡映の積」（Gajardo–Kari–Moreira）。
   その計算可能・有限位数版が本定理。逆向きは破れる（真部分クラス）。
3. **二面体群** — 「n 角形の回転は 2 枚の鏡で作れる」という中学生にも
   説明できる絵が、証明の核（軌道ごとの折り返し）でそのまま使われている。

土台は枯れた数学なので安心して立てる。新規性は「効果的（アルゴリズム的）に
できるか」の一点に集中していて、できる場合（分解定理）とできない場合
（no-go 定理）の**境界線**を自分の手で引ける — これが卒研テーマとしての魅力です。
